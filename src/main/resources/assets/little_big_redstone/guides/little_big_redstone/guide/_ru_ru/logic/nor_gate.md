...
<FloatingColumn width="100" align="right">
    ### Булевый
	Вентили ИЛИ-НЕ всегда выдают только значение 0 (ВЫКЛ) или 1 (ВКЛ).
...
<Row>
	<Column>
		<Recipe id="little_big_redstone:logic/nor_gate" />
	</Column>
...
Вентиль ИЛИ-НЕ — это логический компонент, который выводит сигнал ВЫКЛ только когда **хотя бы один** из входов находится в состоянии ВКЛ.  
Это обратная операция по отношению к [Вентилю ИЛИ](or_gate.md).

Вентиль ИЛИ-НЕ можно настроить на работу с 2–10 входами.
...
<Row>
	<Column>
		<MicrochipScene color="red" includeToolbar={true}>
			...
			<Logic name="nor" x="32" y="16" type="nor_gate" />
			<Logic name="output" x="64" y="16" type="io" data="{config:{input:false,signal_strength:15}}" hide={true} />

			<Wire from="a" fromPort="0" to="nor" toPort="0" />
			<Wire from="b" fromPort="0" to="nor" toPort="1" />
			<Wire from="nor" fromPort="0" to="output" toPort="0" />
	...
	<Column>
		<MicrochipScene color="red" includeToolbar={true}>
			...
			<Logic name="nor" x="32" y="16" type="nor_gate" data="{config:{input_count:4}}" />
			<Logic name="output" x="64" y="24" type="io" data="{config:{direction:'up',input:false,signal_strength:15}}" hide={true} />

			<Wire from="a" fromPort="0" to="nor" toPort="0" />
			<Wire from="b" fromPort="0" to="nor" toPort="1" />
			<Wire from="c" fromPort="0" to="nor" toPort="2" />
			<Wire from="d" fromPort="0" to="nor" toPort="3" />
			<Wire from="nor" fromPort="0" to="output" toPort="0" />
