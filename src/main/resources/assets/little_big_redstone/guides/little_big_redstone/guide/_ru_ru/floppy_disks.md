...
Дискета позволяет легко сохранять программу с [Микрочипа](microchips.md), а затем устанавливать её на другой микрочип.

Нажав **<KeyBind id="key.sneak" />** + **<KeyBind id="key.use" />** на микрочипе,
держа в руке дискету, чтобы сохранить его программу.

После сохранения программы на дискету вы можете нажать **<KeyBind id="key.use" />** на микрочипе, чтобы установить её, при условии, что у вас есть необходимые логические компоненты и редстоун-биты.
Это также можно сделать, держа дискету во второй руке при размещении микрочипа.
Предметы, находящиеся в [Логических матрицах](logic_arrays.md), учитываются как предметы в вашем инвентаре при установке программы.
Необходимые для установки программы предметы будут отображаться над панелью быстрого доступа, когда вы смотрите на микрочип, держа в руке дискету.

Программы также можно сохранять на компьютер и загружать обратно. Откройте меню, нажав **<KeyBind id="key.use" />**, держа в руке дискету (и не глядя на микрочип). 
Кнопка «Сохранить» сохранит текущую программу в файл с именем, которое вы введёте в текстовое поле «Название программы». 
Аналогично, кнопка «Загрузить» загрузит программу из файла с указанным именем. Если файла с таким именем не существует, кнопка «Загрузить» будет неактивна. 
Все файлы хранятся в папке **/little_big_redstone/microchips** в каталоге игры. Эти файлы доступны для разных миров и серверов.