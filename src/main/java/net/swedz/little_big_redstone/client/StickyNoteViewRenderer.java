...
{
	public static void extractBackground(GuiGraphicsExtractor graphics, DyeColor color, boolean pin, float alpha)
	{
		int argb = (Math.round(alpha * 255f) << 24) | 0x00FFFFFF;
		
		var backgroundTexture = LBR.id("sticky_note/background/" + color.getName());
		graphics.blitSprite(RenderPipelines.GUI_TEXTURED, backgroundTexture, 0, 0, 180, 180, argb);
		
		if(pin)
		{
			var pinTexture = LBR.id("sticky_note/pin/" + color.getName());
			graphics.blitSprite(RenderPipelines.GUI_TEXTURED, pinTexture, (180 / 2) - 9, 4, 32, 32, argb);
		}
	}
	
	public static void extractBackground(GuiGraphicsExtractor graphics, StickyNoteView note, float alpha)
	{
		extractBackground(graphics, note.color(), true, alpha);
	}
	
	public static void extractBackground(GuiGraphicsExtractor graphics, StickyNoteView note)
	{
		extractBackground(graphics, note, 1);
	}
	...
	public static void extractText(GuiGraphicsExtractor graphics, DyeColor textColor, Component text, float alpha)
	{
		...
		for(var line : font.split(text, 170))
		{
			...
			graphics.text(font, line, 0, y, argb, false);
			index++;
		}
