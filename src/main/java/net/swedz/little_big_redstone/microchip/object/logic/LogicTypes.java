...
public final class LogicTypes
{
	...
	private static final Codec<Identifier> IDENTIFIER_CODEC = Codec.STRING
			.comapFlatMap(
					(value) ->
					{
						if(Identifier.isValidPath(value))
						{
							return DataResult.success(LBR.id(value));
						}
						return Identifier.read(value);
					},
					Identifier::toString
			)
			.stable();
	 ...
	 * {@link Registry#byNameCodec()}. That uses the vanilla {@link Identifier#CODEC}, which would mean that the
	 * Minecraft namespace would be used by default. This codec instead uses the {@link LogicTypes#IDENTIFIER_CODEC},
	 * which defaults to the LBR namespace instead of the Minecraft one. Otherwise, this codec is merely a replication
	 * of what {@link Registry#byNameCodec()} would return.</p>
	 *
	 * @see Registry#byNameCodec()
	 */
	private static final Codec<LogicType> CODEC = IDENTIFIER_CODEC
			.comapFlatMap(
					(id) -> REGISTRY
							.get(id)
